...
void Item::update(float dt)
{
	...
	if (sincetime >= 3.0f)
	{
		...
		sell_price += random_number;
		buy_price -= random_number;
	}
}
...
Item* ItemBag::getItem(int tag)
{
	switch (tag)
	{
		...
		case 15:
			...
			break;
		case 16:
			return ItemFur::create();
			break;
		case 17:
			return ItemEgg::create();
			break;
		case 50:
...
bool ItemFur::init()
{
	...
	this->num = 1;
	this->name = "Fur";
	this->item = Sprite::create("items/fur.png");
...
bool ItemEgg::init()
{
	...
	this->num = 1;
	this->name = "Egg";
	this->item = Sprite::create("items/egg.png");
	this->item->setScale(1.5);
	this->addChild(this->item);
	return true;
}
...
void ShopBox::update(float dt)
{
	...
	if (sincetime >= 3.0f)
	{
		...
		if (it != nullptr)
		{
			std::string sell_price = std::to_string(it->sell_price);
			sell_price_label->setString("Sell Price: " + sell_price + "G");
			std::string buy_price = std::to_string(it->buy_price);
			buy_price = "Buy Price: " + buy_price + "G";
			buy_price_label->setString(buy_price);
		}
	}
